import * as Tone from 'file:///Users/daniellacosse/code/only-web/node_modules/tone/build/esm/index.js';
...
function getChordRoot(symbol) {
  const [mainDegree, modifierDegree] = symbol.split("/").map(
    (chord) => getScaleInterval(chord.match(/([b#]?[iI]{0,2}[vV]?[iI]{0,2})/)[1])
  );
  if (typeof mainDegree !== "number")
    throw new Error();
  return (mainDegree + (modifierDegree != null ? modifierDegree : 0)) % TONES_IN_OCTAVE;
}
function getChordShape(symbol) {
  ...
  } else if (symbol.toLowerCase() === symbol || symbol.match(/m|min/) && !symbol.match("maj")) {
...
export { parseChord, playChord };
//# sourceMappingURL=index-4066ff15.mjs.map
